    ...
    return Controller.extend("zpe2.pp003.zpe2pp003.controller.ProdOrderView", {
      ...
      _loadKpiData() {
        ...
        oODataModel
          ...
          .catch(() => {
            MessageToast.show(
              "생산오더 KPI 서비스 메타데이터를 불러오지 못했습니다.",
            );
          });
      },
      ...
      _loadYieldData() {
        ...
        oDashboardModel.setProperty(
          "/yield",
          this._createEmptyYieldData(oFromDate),
        );
      ...
      _loadRatioData() {
        ...
        oRatioModel
          ...
          .then(() => {
            oRatioModel.read("/AufnrRatioSet", {
              ...
              success: (oData) => {
                ...
                oDashboardModel.setProperty(
                  "/ratioTotalQty",
                  this._formatQuantity(oRatioData.totalQty),
                );
                oDashboardModel.setProperty(
                  "/ratioUnit",
                  oRatioData.unit || "EA",
          ...
          .catch(() => {
            MessageToast.show(
              "제품별 생산 비율 서비스 메타데이터를 불러오지 못했습니다.",
            );
          });
      },

      _loadTodayOrders() {
        const oTodayModel = this.getOwnerComponent().getModel("today");
        const oDashboardModel = this.getView().getModel("dashboard");

        if (!oTodayModel) {
          MessageToast.show("오늘 생산오더 서비스 모델을 찾을 수 없습니다.");
          return;
        }
        ...
        const oToDate = this._endOfDay(oBaseDate);
        const aFilters = [
          new Filter("Gstrp", FilterOperator.BT, oBaseDate, oToDate),
        ];

        oDashboardModel.setProperty("/todayOrders", []);

        oTodayModel
          .metadataLoaded()
          .then(() => {
            oTodayModel.read("/TodayAufnrSet", {
              filters: aFilters,
              success: (oData) => {
                const aOrders = (oData?.results || [])
                  .filter((oRow) => this._isSameDate(oRow.Gstrp, oBaseDate))
                  .map((oRow, iIndex) =>
                    this._createTodayOrderRow(oRow, iIndex + 1),
                  );

                oDashboardModel.setProperty("/todayOrders", aOrders);
              },
              error: () => {
                MessageToast.show("오늘 생산오더 목록 조회에 실패했습니다.");
              },
            });
          })
          .catch(() => {
            MessageToast.show(
              "오늘 생산오더 서비스 메타데이터를 불러오지 못했습니다.",
            );
          });
      },

      onPressTodayOrder(oEvent) {
        const sAufnr = oEvent
          .getSource()
        ...
        if (sAufnr) {
          MessageToast.show(`${sAufnr} 상세 화면은 추후 연결됩니다.`);
        }
      },

      _createTodayOrderRow(oRow, iNo) {
        const oApstat = this._getApprovalStatus(oRow.Apstat);
        const oStat = this._getProductionStatus(oRow.Stat);
        const oMonStatus = this._getMonitorStatus(
          oRow.MonStatus,
          oRow.MonStatusTxt,
        );
        ...
        return {
          ...
          aufnr: oRow.Aufnr || "",
          maktx: oRow.Maktx || "",
          gamng: this._formatQuantity(oRow.Gamng),
          meins: oRow.Meins || "",
          gstrp: this._formatTodayDate(oRow.Gstrp),
          sname: oRow.sname || oRow.Sname || "-",
          apstatText: oApstat.text,
          apstatState: oApstat.state,
          statText: oStat.text,
          statState: oStat.state,
          monStatusText: oMonStatus.text,
          monStatusState: oMonStatus.state,
        };
      },

      _getApprovalStatus(sApstat) {
        switch (String(sApstat || "")) {
          case "1":
            return { text: "승인대기", state: "Warning" };
          case "2":
            return { text: "승인완료", state: "Success" };
          case "3":
            return { text: "반려", state: "Error" };
          default:
            return { text: "-", state: "None" };
        }
      },

      _getProductionStatus(sStat) {
        switch (String(sStat || "")) {
          case "":
            return { text: "미진행", state: "None" };
          case "1":
            return { text: "진행중", state: "Information" };
          case "2":
            return { text: "완료", state: "Success" };
          case "3":
            return { text: "취소", state: "Error" };
          default:
            return { text: "-", state: "None" };
        }
      },

      _getMonitorStatus(sMonStatus, sMonStatusTxt) {
        const sText = String(sMonStatusTxt || "").trim();
        const oConfig = this._findStatusConfig({
          MonStatus: sMonStatus,
          MonStatusTxt: sText,
        });

        return {
          text: sText || oConfig?.title || "-",
          state: oConfig?.state || "None",
        };
      },

      _formatTodayDate(vDate) {
        const oDate = this._toDate(vDate);
        return oDate ? this._formatDisplayDate(oDate) : "";
      },

      _isSameDate(vDate, oTargetDate) {
        const oDate = this._toDate(vDate);

        return Boolean(
          oDate &&
            oTargetDate &&
            this._formatDisplayDate(oDate) ===
              this._formatDisplayDate(oTargetDate),
        );
      },
      ...
      _buildRatioData(aRows) {
        ...
        const aItems = Object.values(mMaterials)
          ...
          .slice(0, 5);
        const fTotalQty = aItems.reduce(
          (fSum, oItem) => fSum + oItem.prodQty,
          0,
        );
        ...
        return {
          ...
          items: aItems.map((oItem, iIndex) => {
            ...
            return {
              ...
              ratio: fRatio,
              ratioText:
                `${this._formatPercent(fRatio)}% (${this._formatQuantity(oItem.prodQty)} ${oItem.unit || ""})`.trim(),
            };
          }),
        };
      },
      ...
      _buildStatusCards(aRows) {
        ...
        const iTotalCount = Object.values(mByKey).reduce(
          (iSum, oItem) => iSum + oItem.count,
          0,
        );
        ...
        return this._aStatusConfig.map((oConfig) => {
          ...
          return {
            ...
            planQtyText:
              `${this._formatQuantity(oValue.planQty)} ${oValue.unit || ""}`.trim(),
          };
        });
      },
      ...
      _findStatusConfig(oRow) {
        ...
        return this._aStatusConfig.find(
          (oConfig, iIndex) =>
      ...
      _createTotalCard(aKpiData) {
        ...
        return {
          ...
          count: oTotal.count,
          subText:
            `계획수량 ${this._formatQuantity(oTotal.planQty)} ${oTotal.unit || ""}`.trim(),
          planQtyText:
            `${this._formatQuantity(oTotal.planQty)} ${oTotal.unit || ""}`.trim(),
        };
      },
      ...
      _createPeriodText(oBaseDate) {
        ...
        return {
          ...
          ratio: `제품 비율 : 최근 7일 (${this._formatDisplayDate(oSevenFrom)} ~ ${sBase})`,
